...
class TemporaryStorageAdapter:
    ...
    def promote_to_permanent(self, temp_path: str, permanent_identifier: str) -> str:
        ...
        if temp_path not in registry:
            logger.warning(
                "Attempting to promote untracked temporary file: %s", 
                temp_path
            )
    ...
    def _load_registry(self) -> Dict:
        ...
        try:
            ...
            logger.error(
                "Failed to parse registry file as JSON: %s", 
                str(e)
            )
            return {}
            ...
            logger.error(
                "Unexpected error loading registry file: %s", 
                str(e)
            )
            return {}
        ...
        try:
            ...
            for path, info in registry.items():
                try:
                    ...
                    logger.warning(
                        "Invalid expiration data for file %s: %s", 
                        path, 
                        str(e)
                    )
